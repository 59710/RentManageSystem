<template>
  <div class="order-page">
    ...
    <div class="stats-row page-container">
      <div class="stat-card" v-for="(stat, idx) in stats" :key="idx" :class="{ active: currentTab === stat.key }"
    ...
    <div class="order-list page-container">
      ...
      <template v-else-if="orders.length > 0">
        <div class="order-card glass-effect" v-for="order in orders" :key="order.orderId">
          <!-- 卡片头部：状态 + 订单编号 + 时间 -->
          <div class="card-head">
            <div class="head-left">
              <el-tag :type="statusTagMap[order.status]?.type || 'info'" size="large" effect="dark" round
          ...
          <!-- 备注信息 -->
          <div class="remark-block" v-if="order.remark">
          ...
          <div class="card-actions">
            ...
            <el-tag v-else-if="order.status === 3" type="warning" size="small" effect="light" round>
              ❌ 已取消
            </el-tag>
      ...
      <div v-else class="empty-state">
        <el-empty description="暂无租房订单">
          <el-button type="primary" @click="$router.push('/house')">
            去找房
          </el-button>
        </el-empty>
      </div>
    ...
    <el-dialog v-model="payDialogVisible" title="确认支付" width="420px" :close-on-click-modal="false">
      <div class="pay-dialog-body" v-if="payingOrder">
        <div class="pay-summary">
          <p class="pay-title">{{ payingOrder.title }}</p>
          <div class="pay-items">
            <div class="pay-item">
              <span>押金</span><strong>¥{{ payingOrder.deposit?.toLocaleString() }}</strong>
            </div>
            <div class="pay-item">
              <span>首月租金</span><strong>¥{{ payingOrder.rentMonthly?.toLocaleString() }}</strong>
            </div>
            <div class="pay-divider"></div>
            <div class="pay-item pay-total">
              <span>合计支付</span><strong>¥{{ payAmount?.toLocaleString() }}</strong>
            </div>
          </div>
        </div>
        <el-form class="pay-form" @submit.prevent="confirmPay">
...
const currentTab = ref('all') // all | pending | confirmed | completed | cancelled
...
const stats = reactive([
  { key: 'all', label: '全部订单', icon: '📋', color: 'linear-gradient(135deg,#667eea,#764ba2)', count: 0 },
  { key: 'pending', label: '待确认', icon: '⏳', color: 'linear-gradient(135deg,#f093fb,#f5576c)', count: 0 },
  { key: 'confirmed', label: '已确认', icon: '✅', color: 'linear-gradient(135deg,#11998e,#38ef7d)', count: 0 },
  { key: 'completed', label: '已完成', icon: '🏠', color: 'linear-gradient(135deg,#4facfe,#00f2fe)', count: 0 },
])

// 状态映射
const statusTagMap: Record<number, { label: string; type: string; color: string }> = {
  0: { label: '待确认', type: 'warning', color: 'linear-gradient(135deg,#f093fb,#f5576c)' },
  ...
  3: { label: '已取消', type: 'danger', color: 'linear-gradient(135deg,#bdc3c7,#95a5a6)' },
  4: { label: '房东已拒绝', type: 'danger', color: 'linear-gradient(135deg,#e74c3c,#c0392b)' },
}
...
async function fetchOrders() {
  ...
  try {
    ...
    if (currentTab.value !== 'all') {
      const filterMap: Record<string, number> = { pending: 0, confirmed: 1, completed: 2 }
      const targetStatus = filterMap[currentTab.value]
      if (targetStatus !== undefined) {
        // 简单前端过滤（如果后端支持status参数更好）
        // 这里直接用全量数据，因为当前接口不支持按status筛选
      }
    }
...
async function handleCancel(order: RentOrder) {
  try {
    await ElMessageBox.confirm(
      `确定要取消订单 ${order.orderNo} 吗？取消后需重新提交申请。`,
      '取消订单',
...
async function handlePay(order: RentOrder) {
  ...
  try {
    ...
    if (!payment) {
      ElMessage.warning('未找到支付单')
      return
    }
    if (payment.status !== 0) {
      ElMessage.info('该订单已支付或已过期')
      return
    }
